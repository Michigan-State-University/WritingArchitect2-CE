...
class quiz
{
	...
	public function load_completed_quizzes($db, $orgid, $comp)
	{
		...
		while ($row = $stmt->fetch()) {
			...
			$userLastName = htmlspecialchars((string) $row['USER_LAST_NAME'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
			$userFirstName = htmlspecialchars((string) $row['USER_FIRST_NAME'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
			$quiz_list .= "<tr><td align='center'>$edit_link $duplicate_link $complete_link</td><td nowrap>" . $row['Q_GRADING_STATUS'] . "</td><td " . $score_class . ">" . $userLastName . "</td><td " . $score_class . ">" . $userFirstName . "</td><td " . $score_class . ">" . $row['CLASS_NAME'] . "</td><td " . $score_class . ">" . $row['Q_PROMPT_TITLE'] . "</td></tr>";
		}
		$quiz_list .= "</table>";
